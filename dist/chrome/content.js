...
function channelPost(method, url, origin) {
...
function within(a, b, seconds) {
  const gap = Date.parse(b) - Date.parse(a);
  return Number.isFinite(gap) && gap >= 0 && gap <= seconds * 1000;
}
function mergeContent(latest, me, channel, body, seconds, now, lastAppend) {
...
function visualPair(a, b, seconds) {
